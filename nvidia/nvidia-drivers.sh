...
# CachyOS recommends increasing NVIDIA's shader cache size to 12GB for gaming.
echo "Configuring NVIDIA shader cache"
mkdir -p "$HOME/.config/environment.d"
gaming_config="$HOME/.config/environment.d/gaming.conf"
touch "$gaming_config"

if grep -q '^__GL_SHADER_DISK_CACHE_SIZE=' "$gaming_config"; then
    sed -i 's/^__GL_SHADER_DISK_CACHE_SIZE=.*/__GL_SHADER_DISK_CACHE_SIZE=12000000000/' "$gaming_config"
else
